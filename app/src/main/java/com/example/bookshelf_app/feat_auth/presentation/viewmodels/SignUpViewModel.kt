...
class SignUpViewModel @Inject constructor(
            ...
            when(result){
                is ResponseState.Success->{
                    result.data?.let{list ->
                        _mSignUpState.emit(_mSignUpState.value.copy(users = list, containsError = null, isLoading = false))
                    }
                }
                is ResponseState.Error ->{
                    _mSignUpState.emit(_mSignUpState.value.copy(users = listOf(), containsError = result.message, isLoading = false))
                }
                is ResponseState.Loading ->{
                    _mSignUpState.emit(_mSignUpState.value.copy(users = listOf(), containsError = null, isLoading = true))
                }
            }
        }
    }
    fun insertUserList(user: UserModel) = viewModelScope.launch(Dispatchers.IO){
        insertUsers(user).collectLatest {result ->
            when(result){
                is ResponseState.Success->{
                    _mSignUpState.emit(_mSignUpState.value.copy(insertStatus = true, containsError = null, isLoading = false))
                }
                is ResponseState.Error ->{
                    _mSignUpState.emit(_mSignUpState.value.copy(insertStatus = null, containsError = result.message, isLoading = false))
                }
                is ResponseState.Loading ->{
                    _mSignUpState.emit(_mSignUpState.value.copy(insertStatus = null, containsError = null, isLoading = true))
                }
            }
        }
    }
