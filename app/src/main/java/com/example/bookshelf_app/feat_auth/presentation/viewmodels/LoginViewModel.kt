            ...
            when(result){
                is ResponseState.Success->{
                    result.data?.let{list ->
                        _mLoginState.emit(_mLoginState.value.copy(users = list, containsError = null, isLoading = false))
                    }
                }
                is ResponseState.Error ->{
                    _mLoginState.emit(_mLoginState.value.copy(users = listOf(), containsError = result.message, isLoading = false))
                }
                is ResponseState.Loading ->{
                    _mLoginState.emit(_mLoginState.value.copy(users = listOf(), containsError = null, isLoading = true))
                }
            }
        }
    }
